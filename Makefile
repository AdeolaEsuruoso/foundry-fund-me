...
.PHONY: all test clean remove update build zkbuild deploy deploy-sepolia deploy-zk deploy-zk-sepolia fund withdraw help install snapshot format anvil zk-anvil zktest
...
# deploy-sepolia always targets Sepolia, no ARGS needed
deploy-sepolia:
...
# For deploying Interactions.s.sol:FundFundMe / WithdrawFundMe we need --sender <ADDRESS>
